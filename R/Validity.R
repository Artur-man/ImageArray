.validate_ImageArray <- function(object){
  ...
  if(!"axes" %in% names(meta(object)))
    stop("'axes' should not be provided in the metadata of the 
  ...
  if(!all(axes(object) %in% .AXES))
    stop(
      sprintf("The axes of the ImageArray object should be 
               a subset of %s%"), 
      .AXES
    )
  ...
  if(!all(all_dim == length(axes(object))))
    stop("The number of dimensions of all levels 
         should match the number of axes.")
