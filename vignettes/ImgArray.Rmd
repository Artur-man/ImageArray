...
The main usage of the `r Biocpkg("ImageArray")` is to deliver `r Biocpkg("DelayedArray")` operations for 
images or a list of images in Pyramidal format. You can also use
`magick-image` objects.

```{r read}
# make random EBImage image
arr <- array(data=sample(1:255, 2000*5000*3, replace = TRUE), 
             dim=c(3, 2000, 5000))
...
```{r crop2}
imgarray_cropped <- crop(imgarray, ind = list(2001:3000, 501:1000))
imgarray_cropped
```
...
ome.tiff.file <- system.file("extdata", "xy_12bit__plant.ome.tiff", 
                             package = "ImageArray")
imgarray <- createImgArray(ome.tiff.file, n.series = 1, resolution = 1:2)
imgarray
```
